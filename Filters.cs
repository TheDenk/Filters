...
namespace ImageManipulate
{
    public static class Filter
    {
        ...
        public static Bitmap ApplyHafAlgoritm(Bitmap source, out int[,] halfArray, int incrementOfOutArray = 1)
        {
            var width = source.Width;
            var height = source.Height;
        ...
        public static void TestMetod(Bitmap source, int[,] halfArray, out Rectangle rect, double level, int minW = 120, int minH = 20)
        {
            var width = source.Width;
            var height = source.Height;
            ...
            var verticalNormals = new List<int>();
            var verticalAngles = new List<int>();
            var horizontalNormals = new List<int>();
            var horizontalAngles = new List<int>();
            ...
            for (int i = 0; i < halfArray.GetLength(0); i++)
            {
                ...
                {
                    if (((j - 90) > -5 && (j - 90) < 5))
                        if (halfArray[i, j] > level)
                        {
                            verticalNormals.Add(i);
                            verticalAngles.Add(j);
                        }
                    if (((j - 90) >= (-90) && (j - 90) <= (-85)) || ((j - 90) >= 85 && (j - 90) <= 90))
                    {
                        if (halfArray[i, j] > level * 3)
                        {
                            horizontalNormals.Add(i);
                            horizontalAngles.Add(j);
                        }
                    }
                }
            }   // сбор горизонтальных и вертикальных прямых в списки

            var equalsVerricalAngles = new List<int>();
            var equalsVerricalNormalsRight = new List<int>();
            var equalsVerricalNormalsLeft = new List<int>();
            var normalBetweenVertical = new List<int>();

            for (int i = 0; i < verticalAngles.Count; i++)
            {
                for (int j = 0; j < verticalAngles.Count; j++)
                {
                    int p = verticalNormals[i] - verticalNormals[j];
                    if ((p > minW) && (verticalAngles[i] == verticalAngles[j]))
                    {
                        equalsVerricalAngles.Add(verticalAngles[i]);
                        equalsVerricalNormalsRight.Add(verticalNormals[i]);
                        equalsVerricalNormalsLeft.Add(verticalNormals[j]);
                        normalBetweenVertical.Add(p);
                    }
                }
            }

            var equalsHorizontalAngles = new List<int>();
            var equalsHorizontalNormalsTop = new List<int>();
            var equalsHorizontalNormalsBot = new List<int>();
            var normalBetweenHorizontal = new List<int>();

            for (int i = 0; i < horizontalAngles.Count; i++)
            {
                for (int j = 0; j < horizontalAngles.Count; j++)
                {
                    int p = horizontalNormals[i] - horizontalNormals[j];
                    if ((p > minH) && (horizontalAngles[i] == horizontalAngles[j]))
                    {
                        equalsHorizontalAngles.Add(horizontalAngles[i]);
                        equalsHorizontalNormalsTop.Add(horizontalNormals[i]);
                        equalsHorizontalNormalsBot.Add(horizontalNormals[j]);
                        normalBetweenHorizontal.Add(p);
                    }
                }
            }
            var finalNormalsV = new List<int>();
            var finalAnglesV = new List<int>();
            var finalNormalsH = new List<int>();
            var finalAnglesH = new List<int>();

            int hAngle = Math.Abs(equalsHorizontalAngles[0] - 90),
                vAngle = Math.Abs(equalsVerricalAngles[0] - 90),
                leftEdge = equalsVerricalNormalsLeft[0],
                rightEdge = equalsVerricalNormalsRight[0],
                botEdge = equalsHorizontalNormalsBot[0],
                topEdge = equalsHorizontalNormalsTop[0],
                normalH = normalBetweenHorizontal[0],
                normalV = normalBetweenVertical[0];


            for (int i = 0; i < normalBetweenVertical.Count; i++)
            {
                for (int j = 0; j < normalBetweenHorizontal.Count; j++)
                {
                    double eq = normalBetweenVertical[i] / normalBetweenHorizontal[j];

                    if ((eq > 3) && (eq < 6))
                    {

                        finalNormalsV.Add(equalsVerricalNormalsRight[i]);
                        finalNormalsV.Add(equalsVerricalNormalsLeft[i]);
                        finalAnglesV.Add(equalsVerricalAngles[i]);
                        finalAnglesV.Add(equalsVerricalAngles[i]);

                        finalNormalsH.Add(equalsHorizontalNormalsTop[j]);
                        finalNormalsH.Add(equalsHorizontalNormalsBot[j]);
                        finalAnglesH.Add(equalsHorizontalAngles[j]);
                        finalAnglesH.Add(equalsHorizontalAngles[j]);

                        //if(Math.Abs(equalsHorizontalAngles[j] - 90) >= hAngle)
                        if ((normalBetweenHorizontal.Count / (j + 1) == 2))
                        {
                            normalH = normalBetweenHorizontal[j];
                            hAngle = equalsHorizontalAngles[j];
                            botEdge = equalsHorizontalNormalsBot[j];
                            topEdge = equalsHorizontalNormalsTop[j];
                        }
                        // if (Math.Abs(equalsVerricalAngles[i] - 90) <= vAngle)
                        if ((normalBetweenVertical.Count / (i + 1) == 2))
                        {
                            normalV = normalBetweenVertical[i];
                            vAngle = equalsVerricalAngles[i];
                            leftEdge = equalsVerricalNormalsLeft[i];
                            rightEdge = equalsVerricalNormalsRight[i];
                        }
                    }
                }
            }
            int xRect = leftEdge, yRect = topEdge, hRect = rightEdge - leftEdge, wRect = botEdge - topEdge;
            unsafe
            {
                for (var y = 0; y < height; y++)
                {
                    var sourceRow = (byte*)sourceScan0 + (y * sourceStride);
                    {
                        double pVerticalLeft = leftEdge - halfArray.GetLength(0) / 2;
                        double pVerticalRight = rightEdge - halfArray.GetLength(0) / 2;

                        double cosV = sinCos[1, vAngle];
                        double sinV = sinCos[0, vAngle];

                        double cosH = sinCos[1, hAngle];
                        double sinH = sinCos[0, hAngle];

                        int xVL = (int)(pVerticalLeft / cosV - y * sinV / cosV);
                        int xVR = (int)(pVerticalRight / cosV - y * sinV / cosV);
                        
                        if (xVL < width && xVL >= 0)
                        {
                            sourceRow[xVL * resultPixelSize] = 0;
                            sourceRow[xVL * resultPixelSize + 1] = 0;
                            sourceRow[xVL * resultPixelSize + 2] = 255;
                        }
                    }
                }
            }
        ...
        public static Bitmap ApplyDoubleTresholding2(Bitmap source, int lowLevel, int highLevel)
        {
            var width = source.Width;
            var height = source.Height;
            ...
            unsafe
            {
                for (var y = 0; y < height - 0; y++)
                {
                    ...
                    for (var x = 0; x < width - 0; x++)
                    {
                        var v = (byte)(sourceRow[x * resultPixelSize] > highLevel  ? 255 :
                            sourceRow[x * resultPixelSize] < lowLevel ? 0 : 127);
                        resultRow[x * resultPixelSize] = v;
                        resultRow[x * resultPixelSize + 1] = v;
                        resultRow[x * resultPixelSize + 2] = v;
